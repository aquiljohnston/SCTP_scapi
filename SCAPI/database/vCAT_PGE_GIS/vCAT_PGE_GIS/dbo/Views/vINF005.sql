...
,CASE WHEN CLB.MPRStatus IS NULL THEN UPPER(CAST(svor.UserLANID AS VARCHAR(4))) ELSE NULL END AS [SRVY_LANID]

,UPPER(COALESCE(CAST(sup.UserLANID AS VARCHAR(4)),'SMFF')) AS [SPVR_LANID]  -- This proxy for SMFF needs to be removed ASAP. ****************************************************
...
,CASE WHEN CLB.MPRStatus IS NULL AND CLB.EquipmentType = 'G_COGIDPIR' AND CLB.CalibrationVerificationFlag = 1 THEN CAST(CAST(CLB.ReadPPM AS NUMERIC(6,2)) AS VARCHAR(7)) ELSE NULL END AS [DPIR_READ_PPM]
...
,CASE WHEN CLB.MPRStatus IS NULL AND CLB.EquipmentType = 'G_COGIRMLD' AND CLB.CalibrationVerificationFlag = 1 THEN CAST(CAST(CLB.ReadPPM AS NUMERIC(6,2)) AS VARCHAR(7)) ELSE NULL END AS [RMLD_READ_PPM]
...
,CASE WHEN CLB.MPRStatus IS NULL AND CLB.EquipmentType = 'G_COGIFMPK' AND CLB.CalibrationVerificationFlag = 1 THEN CAST(CAST(CLB.ReadPPM AS NUMERIC(6,2)) AS VARCHAR(7)) ELSE NULL END AS [FPMK_READ_PPM]

,CASE WHEN CLB.MPRStatus IS NULL AND CLB.EquipmentType = 'G_COGIOMD'  AND CLB.CalibrationVerificationFlag = 1 THEN CAST(CAST(CLB.OMDExmQty AS NUMERIC(6,2)) AS VARCHAR(7)) ELSE NULL END AS [OMD_EXMN_QTY]
,CASE WHEN CLB.MPRStatus IS NULL AND CLB.EquipmentType = 'G_COGIOMD'  AND CLB.CalibrationVerificationFlag = 1 THEN CAST(CAST(CLB.ReadPPM AS NUMERIC(6,2)) AS VARCHAR(7)) ELSE NULL END AS [OMD_READ_PPM]
...
,CASE WHEN CLB.MPRStatus IS NULL AND CLB.EquipmentType = 'G_COGISCOP' AND CLB.CalibrationVerificationFlag = 1 THEN CAST(CAST(CLB.PLELRead AS NUMERIC(6,2)) AS VARCHAR(7)) ELSE NULL END AS [SCOP_PLEL_RDG]
,CASE WHEN CLB.MPRStatus IS NULL AND CLB.EquipmentType = 'G_COGISCOP' AND CLB.CalibrationVerificationFlag = 1 THEN CAST(CAST(CLB.PGASRead AS NUMERIC(6,2)) AS VARCHAR(7)) ELSE NULL END AS [SCOP_PGAS_RDG]
...
LEFT  JOIN [dbo].[rWorkCenter] wc
	ON wc.WorkCenterAbbreviation = el.MWC
